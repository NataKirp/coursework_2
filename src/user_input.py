...
def get_valid_altitude_range(prompt: str, allow_empty: bool = False) -> Optional[tuple[float, float]]:
    """
    ...
    while True:
        ...
        if not range_input and allow_empty:
            return None
        parts = re.split(r'[-,\s]+', range_input)  # режем строку (пробелы, запятые, дефисы)
        parts = list(filter(None, parts))  # чистим от пустых строк (если введено несколько разделителей подряд
        ...
        try:
            ...
            print("❌ Ошибка: введите корректные числовые значения (например, 10000 - 15000).")
