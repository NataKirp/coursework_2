...
def get_top_airplanes(sorted_airplanes: list[dict[str, any]], top_n: int) -> list[dict[str, any]]:
    ...
    if not top_airplanes:
        ...
        print(f"Найдено всего {count_planes} самолета(ов), что меньше запрошенных {top_n}. Выводим все найденные:")
    else:
        print(f"Выводим топ-{top_n} самолетов:")
    return top_airplanes


def get_top_by_altitude(json_saver, alt_range: tuple, top_n: int) -> list[Airplane]:
...
def print_airplanes(top_airplanes: list[Airplane]) -> None:
    if not top_airplanes:
        return
    print(
        f"{'ID борта':<15} | {'Позывной рейса':<15} | {'Страна регистрации ВС':<30} | {'Высота':<10} | {'Скорость':<10}")
    print("-" * 92)
    for p in top_airplanes:
        print(f"{p.icao24:<15} | {p.callsign:<15} | {p.country:<30} | {p.geo_altitude:>10.2f} | {p.velocity:>10.2f}")
